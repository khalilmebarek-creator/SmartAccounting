...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const translations = {
        ar: {
            ...
            hero_badge: 'v3.2.0 — مشروع تخرج محاسبي',
    ...
    dl_mob_title: 'تطبيق الجوال (Android)',
    dl_mob_desc: 'رفيق المتابعة: لوحة قيادة + 20 نسبة + صحة AI + التقويم الجبائي',
    dl_mob_btn: 'تحميل APK',
            dl_recommended: 'مُوصى به',
            dl_note_sac: '<strong>ملاحظة لمستخدمي Windows 11:</strong> إذا ظهرت رسالة "An Application Control policy has blocked this file" عند فتح التطبيق، فالمشكلة من ميزة <em>Smart App Control</em> — يُحل بإيقافها من: <code>Windows Security ← App &amp; browser control ← Smart App Control settings ← Off</code> أو بتشغيل السكربت المساعد <code>tools\allow_smart_app_control.ps1</code> كمسؤول (يفحص الحالة ويضيف استثناء Defender). التطبيق يدعم Windows 10 و 11، والمشكلة خاصة ببعض أجهزة Windows 11 فقط.',
            ...
            upd_new28: 'تغطية واجهات شاملة: 111 اختباراً جديداً يغطي كل الشاشات الـ35 + إصلاح بغّ ألوان جدول المعاملات في المزامنة البنكية + إضافة 44 مفتاح ترجمة مفقوداً للمساعد المحلي بالعربية (176 مفتاحاً جديداً × 3 لغات) — 1772 اختباراً ناجحاً + تغطية وحدات 100%',
            upd_new29: 'تحسينات UI حسب ملاحظات المستخدم: أسماء الألوان المترجمة في لوحة التحكم المتقدمة بدل الرموز + تكبير جداول مراكز التكلفة وسجل المزامنة السحابية والمعاملات الشهرية للشركات التجريبية + حقول وحوانب أوسع في شاشة اختبار المستخدمين + عزل اختبارات الواجهة عن بيانات المستخدم المحفوظة',
            upd_new30: 'اختبار UAT شامل كمستخدمين حقيقيين: رحلة مستخدم كاملة عبر التطبيق (تسجيل دخول + تجوّل الشاشات الـ35 + إدخال بيانات + تبديل 3 لغات + حفظ + تسجيل خروج) — 1787 اختباراً ناجحاً + كشف وإصلاح بغّ حقيقي عند تبديل اللغة في شاشتي المزامنة البنكية واستيراد البيانات',
            upd_new31: 'معالجة Smart App Control على Windows 11: سكربت مساعد (allow_smart_app_control.ps1) يفحص حالة الحماية ويضيف استثناءات Defender ويعرض تعليمات الإيقاف — حل حجب الـ exe غير الموقّع',
            upd_new32: 'إصلاح الشاشات الفارغة في النسخ المثبّتة: التحميل الكسول للمشاهد والمحرّكات لم يكن مُضمّناً في بناء Nuitka فأصبح الـ exe يفتقر إليها — أُضيفت عبر include-package وأُعيد بناء المثبّتات (مثبّت 66.9MB / ZIP محمول 109MB)',
            upd_new33: 'إصلاح مشاكل النسخة المثبّتة v3.1.7: الشاشات الفارغة (الترقية وصلت ناقصة ملفات pandas/sqlite3/pytz) + تجمّد تحليل DuPont عند العودة + عرض 0.00 قبل الإدخال أصبح «أدخل المبلغ» + الترقية الآن تحافظ على بيانات المستخدم وكلمة السر (استثناء ملفات الحالة من المثبّت) — 1787 اختباراً ناجحاً',
            upd_date9: '4 أغسطس 2026',
    upd_date10: '13 أغسطس 2026',
            upd_new34: 'النظام الجبائي السنوي: تبويب «سنوات النظام» في شاشة الضرائب — إضافة/تبديل/حذف سنة جبائية من الواجهة + محرر JSON بالتحقق والحفظ والاستيراد والتصدير — تصحيح شرائح IRG 2026 حسب نشرة DGI الرسمية (0/23/27/30/33/35%) + النظم الجديدة: IFU (0.5%/5%/12% بحد أدنى 30,000 دج) + رسم التكوين المهني والتمهين (1% + 1%) + الاقتطاع من المصدر على الإيجارات (7%/15%) — 1862 اختباراً ناجحاً',
            upd_new35: 'ملف إعدادات مستقل لكل سنة جبائية (config_years/) مع دمج تلقائي مع الافتراضية — كل ملف قديم يحصل على الأقسام الجديدة دون كسر — لا حاجة لإعادة البناء عند صدور نشرات DGI المستقبلية',
    upd_new36: 'تطبيق جوال Android (v1.1): 5 شاشات — لوحة قيادة برسوم + 20 نسبة + صحة AI برادار مخاطر + تقويم جبائي بتنبيهات + IAS — يقرأ ملفات snapshot من سطح المكتب حتى المشفّرة (AES-256-GCM/Argon2id) بأرقام مطابقة حرفياً، 3 لغات RTL، 53 اختباراً',
    upd_new37: 'منصة ذكاء اصطناعي متكاملة (شاشة 39): درجة صحة مالية 0-100 من 6 محاور + رادار مخاطر + ملخص تنفيذي + توصيات استراتيجية + تقارير IAS/IFRS (شاشة 38): مركز مالي + دخل شامل + تدفقات نقدية + حقوق ملكية',
    upd_new38: 'تفعيل ML كاملاً + شريط Ribbon بسبعة تبويبات + إصلاح انهيار matplotlib في الثيم الداكن — 2037 اختباراً على سطح المكتب + 53 على الجوال',
        ...
        en: {
            ...
            nav_updates: 'Updates',
            hero_badge: 'v3.2.0 — Master\'s Graduation Project',
    ...
    dl_mob_title: 'Mobile App (Android)',
    dl_mob_desc: 'Companion: dashboard + 20 ratios + AI health + tax calendar',
    dl_mob_btn: 'Download APK',
            dl_recommended: 'Recommended',
            dl_note_sac: '<strong>Note for Windows 11 users:</strong> If you see "An Application Control policy has blocked this file" when opening the app, it\'s caused by the <em>Smart App Control</em> feature — fix it by turning it off: <code>Windows Security ← App &amp; browser control ← Smart App Control settings ← Off</code>, or run the helper script <code>tools\allow_smart_app_control.ps1</code> as administrator (checks the state and adds a Defender exclusion). The app supports Windows 10 &amp; 11; this only affects some Windows 11 devices.',
            ...
            upd_new28: 'Comprehensive UI coverage: 111 new tests covering all 35 screens + fixed a color bug in the bank-sync transactions table + added 44 missing i18n keys for the Arabic local assistant (176 new keys across 3 languages) — 1772 passing tests + 100% module coverage',
            upd_new29: 'UI improvements from user feedback: localized accent color names in the advanced dashboard instead of codes + larger tables in cost-center profitability, cloud-sync history and demo-company monthly transactions + wider fields in the user testing screen + UI tests isolated from saved user data',
            upd_new30: 'Comprehensive UAT as real users: a full user journey through the app (login + navigating all 35 screens + data entry + switching 3 languages + saving + logout) — 1787 passing tests + discovered and fixed a real bug when switching language on the bank-sync and data-import screens',
            upd_new31: 'Smart App Control handling on Windows 11: a helper script (allow_smart_app_control.ps1) checks protection status, adds Defender exclusions and shows disable instructions — solves unsigned-EXE blocking',
            upd_new32: 'Fixed blank screens in the built installers: the lazy-loaded views and module engines were not included in the Nuitka build so the EXE was missing them — now bundled via include-package and installers rebuilt (Installer 66.9MB / portable ZIP 109MB)',
            upd_new33: 'Fixed v3.1.7 installed-version issues: blank screens (the update arrived missing pandas/sqlite3/pytz files) + DuPont analysis freezing when returning + 0.00 shown before input is now "Enter amount" + upgrades now keep your user data and password (state files excluded from the installer) — 1787 passing tests',
            upd_date9: 'August 4, 2026',
    upd_date10: 'August 13, 2026',
            upd_new34: 'Annual tax system: new "Tax Years" tab in the tax screen — add/switch/delete a fiscal year from the UI + JSON editor with validation, save, import & export — IRG 2026 brackets fixed per the official DGI bulletin (0/23/27/30/33/35%) + new regimes: IFU (0.5%/5%/12%, 30,000 DZD minimum) + professional training & apprenticeship levy (1% + 1%) + tax withholding on rents (7%/15%) — 1862 passing tests',
            upd_new35: 'One settings file per fiscal year (config_years/) with automatic deep-merge against defaults — any legacy config picks up the new sections without breaking — no rebuild needed for future DGI bulletins',
    upd_new36: 'Android mobile app (v1.1): 5 screens — dashboard with charts + 20 ratios + AI health with risk radar + tax calendar with alerts + IAS — reads desktop snapshot files, even encrypted (AES-256-GCM/Argon2id) with identical figures, 3 languages RTL, 53 tests',
    upd_new37: 'Integrated AI platform (screen 39): financial health score 0-100 across 6 dimensions + risk radar + executive summary + strategic recommendations + IAS/IFRS reports (screen 38): balance sheet + comprehensive income + cash flow + equity',
    upd_new38: 'ML fully enabled + 7-tab Ribbon + fixed matplotlib crash on dark theme — 2037 desktop tests + 53 mobile tests',
        ...
        fr: {
            ...
            hero_badge: 'v3.2.0 — Projet de fin d\'étude Master',
    ...
    dl_mob_title: 'Application mobile (Android)',
    dl_mob_desc: 'Compagnon : tableau de bord + 20 ratios + santé IA + calendrier fiscal',
    dl_mob_btn: 'Télécharger APK',
            dl_recommended: 'Recommandé',
            dl_note_sac: '<strong>Note pour les utilisateurs de Windows 11 :</strong> Si le message "An Application Control policy has blocked this file" s\'affiche à l\'ouverture de l\'application, c\'est dû à la fonction <em>Smart App Control</em> — désactivez-la : <code>Windows Security ← App &amp; browser control ← Smart App Control settings ← Off</code> ou lancez le script d\'assistance <code>tools\allow_smart_app_control.ps1</code> en tant qu\'administrateur (vérifie l\'état et ajoute une exclusion Defender). L\'application prend en charge Windows 10 et 11 ; ce problème ne touche que certains appareils Windows 11.',
            ...
            upd_new28: 'Couverture UI complète : 111 nouveaux tests couvrant les 35 écrans + correction d\'un bug de couleurs dans le tableau des transactions de la synchro bancaire + ajout de 44 clés i18n manquantes pour l\'assistant local en arabe (176 nouvelles clés sur 3 langues) — 1772 tests réussis + couverture des modules 100%',
            upd_new29: 'Améliorations UI selon les retours utilisateurs : noms de couleurs localisés dans le tableau de bord avancé au lieu des codes + tableaux agrandis dans la rentabilité des centres de coûts, l\'historique de synchro cloud et les transactions mensuelles des sociétés de démo + champs plus larges dans l\'écran de tests utilisateurs + tests UI isolés des données utilisateur sauvegardées',
            upd_new30: 'UAT complet en tant qu\'utilisateurs réels : parcours utilisateur complet dans l\'application (connexion + navigation dans les 35 écrans + saisie de données + basculement de 3 langues + enregistrement + déconnexion) — 1787 tests réussis + découverte et correction d\'un vrai bug lors du changement de langue sur les écrans de synchro bancaire et d\'import de données',
            upd_new31: 'Gestion du Smart App Control sous Windows 11 : un script d\'assistance (allow_smart_app_control.ps1) vérifie l\'état de la protection, ajoute des exclusions Defender et affiche les instructions d\'arrêt — solution au blocage de l\'EXE non signé',
            upd_new32: 'Correction des écrans vides dans les installateurs : les vues chargées en lazy-load et les moteurs de modules n\'étaient pas inclus dans le build Nuitka donc absents de l\'EXE — désormais intégrés via include-package et installateurs reconstruits (Installeur 66,9Mo / ZIP portable 109Mo)',
            upd_new33: 'Correction des problèmes de la version installée v3.1.7 : écrans vides (la mise à jour arrivait sans les fichiers pandas/sqlite3/pytz) + gel de l\'analyse DuPont au retour + le 0,00 affiché avant la saisie devient « Saisir le montant » + les mises à jour préservent maintenant vos données utilisateur et votre mot de passe (fichiers d\'état exclus de l\'installateur) — 1787 tests réussis',
            upd_date9: '4 août 2026',
    upd_date10: '13 août 2026',
            upd_new34: 'Système fiscal annuel : nouvel onglet « Années fiscales » dans l\'écran des impôts — ajouter/changer/supprimer une année fiscale depuis l\'interface + éditeur JSON avec validation, sauvegarde, import et export — tranches IRG 2026 corrigées selon le bulletin officiel de la DGI (0/23/27/30/33/35 %) + nouveaux régimes : IFU (0,5 %/5 %/12 %, minimum 30 000 DA) + taxe de formation professionnelle et d\'apprentissage (1 % + 1 %) + retenue à la source sur loyers (7 %/15 %) — 1862 tests réussis',
            upd_new35: 'Un fichier de configuration par année fiscale (config_years/) avec fusion automatique avec les valeurs par défaut — tout ancien fichier reçoit les nouvelles sections sans casse — aucune reconstruction nécessaire pour les futurs bulletins de la DGI',
    upd_new36: 'Application mobile Android (v1.1) : 5 écrans — tableau de bord avec graphiques + 20 ratios + santé IA avec radar des risques + calendrier fiscal avec alertes + IAS — lit les snapshots du bureau, même chiffrés (AES-256-GCM/Argon2id) avec des chiffres identiques, 3 langues RTL, 53 tests',
    upd_new37: 'Plateforme IA intégrée (écran 39) : score de santé financière 0-100 sur 6 dimensions + radar des risques + résumé exécutif + recommandations + rapports IAS/IFRS (écran 38) : bilan + résultat global + flux de trésorerie + capitaux propres',
    upd_new38: 'ML entièrement activé + ruban à 7 onglets + correction du crash matplotlib en thème sombre — 2037 tests bureau + 53 tests mobile',
