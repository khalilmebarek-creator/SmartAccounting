...
> نقطة الدخول الموحّدة لكل وثائق المشروع. الإصدار v3.1.8.
...
| الوثيقة | الوصف |
|---------|-------|
| [`USER_GUIDE.md`](USER_GUIDE.md) | دليل المستخدم: كل الشاشات الـ 35 + ميزات جديدة + أفضل الممارسات + FAQ |
...
| [`TAX_SYSTEM_YEARS.md`](TAX_SYSTEM_YEARS.md) | إصدارات النظام الجبائي السنوية (نشرة DGI 2026: IRG/IFU/رسم التكوين/الاقتطاع) |
...
**ما الإصدار الحالي؟** v3.1.8 (إعدادات الإصدار: 3.1.8 في `config.py`).

**كم عدد الاختبارات؟** 1862 اختباراً ناجحاً + تغطية وحدات 100% (`modules/` بالكامل).
...
**كيف أحدّث النظام الجبائي عند صدور نشرة DGI جديدة؟** شاشة الضرائب ← تبويب "سنوات النظام" ← إضافة سنة (تُنسخ الأحدث) → عدّل في محرر JSON → تحقّق → حفظ → تفعيل. التفاصيل: `docs/TAX_SYSTEM_YEARS.md`.

**ما الشاشات الجديدة في v3.1.7؟** دفتر الأستاذ (F10) + حسابات الأطراف (F11) + الفواتير (F12) + المخزون (Ctrl+Shift+B) + الرواتب (Ctrl+Shift+C) + الميزانية التفصيلية (Ctrl+Shift+D) — إجمالي 35 شاشة.
...
*آخر تحديث: 2026-08-04 — مراجعة شاملة للجاهزية (ISO).*
