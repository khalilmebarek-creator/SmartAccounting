...
def build():
    ...
    ar_para(doc, "الإصدار: v3.1.8", size=14, bold=True, space_after=4)
    ...
    ar_para(doc, "تم تطوير المنصة باستخدام لغة Python وإطار العمل PyQt6، مع الاعتماد على أحدث التقنيات في تحليل البيانات والرسوم البيانية والتقارير. المشروع هو تطبيق سطح مكتب (Desktop Application) يعمل على نظام Windows 10/11، مع إمكانية تشغيله بشكل محمول (Portable) أو من خلال مثبت (Installer).")
    ar_para(doc, "وصل المشروع إلى الإصدار v3.1.7 بعد جلسات تطوير متتالية أضافت ميزات متقدمة شملت تحليل DuPont، تحليل السيناريوهات، المعايير المرجعية، لوحة التحكم المتقدمة، الامتثال الضريبي الكامل، محرك الرؤى الذكية، تحليل مراكز التكلفة، تعدد العملات، المزامنة السحابية، الشركات التجريبية، واختبار UAT شامل (2020 اختباراً ناجحاً).")
    ...
    bullet(doc, "د. توفير 35 شاشة تفاعلية بواجهة سهلة الاستخدام مع اختصارات لوحة مفاتيح كاملة")
    bullet(doc, "هـ. دعم 3 لغات: العربية، الإنجليزية، الفرنسية مع تبديل فوري (2018 مفتاحاً للترجمة)")
    ...
    bullet(doc, "ح. توفير 2020 اختبار وحدة ناجحاً مع تغطية 100% للوحدات البرمجية")
    ...
    bullet(doc, "4. تطوير واجهات المستخدم (35 شاشة)")
    ...
    bullet(doc, "7. الاختبار والتحسين المستمر (2020 اختباراً + تغطية 100%)")
    ...
    bullet(doc, "طبقة العرض (UI): 35 شاشة عبر PyQt6 مع تحميل كسول (Lazy Loading)")
    bullet(doc, "طبقة المنطق التجاري (Modules): أكثر من 37 وحدة برمجية متخصصة")
    ...
    heading(doc, "8.2 PyQt6 — إطار عمل واجهات المستخدم", 2)
    ar_para(doc, "يتم إنشاء MainWindow (QMainWindow) عند بدء التشغيل. تحتوي على QStackedWidget يضم 35 شاشة (QWidget). التنقل بين الشاشات يتم عبر QListWidget جانبي. يتم تحميل الشاشات عند الطلب (Lazy Loading) لتحسين الأداء.")
    ...
    ar_para(doc, "مكتبة الرسوم البيانية الأساسية. تُستخدم في لوحة التحكم، الرادار، شلال DuPont، حساسية Tornado، خطوط التنبؤ، ومخططات مراكز التكلفة. يتم دمجها مع PyQt6 عبر FigureCanvasQTAgg وتحديثها تلقائياً عند تغير البيانات.")
    ...
    ar_para(doc, "مترجم Python يحول الكود إلى C ثم يجمعه إلى ملف تنفيذي. أنتج نسخة مستقلة (Standalone EXE) حجمها 143MB. تم تحسين الإقلاع من 778ms إلى 44ms وتقليل استهلاك الذاكرة من 128MB إلى 45MB.")
    heading(doc, "8.8 Inno Setup — إنشاء المثبت", 2)
    ar_para(doc, "أداة إنشاء مثبتات Windows. توزّع التطبيق بمثبت حجمه 66.9MB ونسخة ZIP بحجم 109.4MB. الميزات: AppMutex لضمان إغلاق التطبيق، CloseApplications=force، وتحديث صامت.")
    ...
    bullet(doc, "modules/ledger.py: ميزان المراجعة والأستاذ العام (شاشة 30)")
    bullet(doc, "modules/partners.py: تقادم ديون العملاء والموردين (شاشة 31)")
    bullet(doc, "modules/invoicing.py: إدارة الفواتير وحساب TVA (شاشة 32)")
    bullet(doc, "modules/inventory.py: إدارة المخزون بمتوسط التكلفة (شاشة 33)")
    bullet(doc, "modules/payroll.py: كشف الرواتب CNAS+IRG (شاشة 34)")
    bullet(doc, "modules/budgeting.py: مقارنة الميزانية الفعلية والمخطط (شاشة 35)")
    bullet(doc, "modules/tax_years.py: إدارة سنوات النظام الجبائي (2025/2026/... مع تحقق وتصدير)")
    bullet(doc, "modules/commercial/: الترخيص والتشفير وطبقات الاستخدام (RSA-2048 + Argon2id + بوابة الميزات)")

    # ============================ 10. واجهات المستخدم ============================
    heading(doc, "10. واجهات المستخدم", 1)
    ar_para(doc, "يحتوي النظام على 35 شاشة تفاعلية بواجهة سهلة الاستخدام، مع دعم كامل للغة العربية من اليمين لليسار (RTL).")
    heading(doc, "10.1 المميزات العامة", 2)
    bullet(doc, "دعم 3 لغات (عربي RTL، إنجليزي LTR، فرنسي LTR) مع تبديل فوري (2018 مفتاح ترجمة)")
    bullet(doc, "3 ثيمات: فاتح (Light)، داكن (Dark)، عصري (Modern) مع تحسينات تباين وإتاحة")
    bullet(doc, "اختصارات كاملة: Ctrl+1..0 و Ctrl+Shift+1..0 و F2..F12 للشاشات 21-35 و Ctrl+T للثيم و F1 للاختصارات")
    bullet(doc, "قائمة عرض (View) ديناميكية بكل الشاشات 35")
    ...
    heading(doc, "10.2 الشاشات الـ35", 2)
    make_table(doc,
        ["#", "الشاشة", "#", "الشاشة"],
        [
            ["1", "لوحة التحكم", "19", "الطباعة والتصدير"],
            ["2", "إدخال البيانات", "20", "نقطة التعادل"],
            ["3", "النسب المالية (20 + Z-Score)", "21", "مراكز التكلفة"],
            ["4", "التقارير المحفوظة", "22", "مقارنة الميزانية"],
            ["5", "النسخ الاحتياطي", "23", "لوحة التحكم المتقدمة"],
            ["6", "استيراد البيانات", "24", "الرؤى الذكية AI"],
            ["7", "الإعدادات", "25", "ربحية مراكز التكلفة"],
            ["8", "المستخدمون", "26", "تعدد العملات"],
            ["9", "النظام الجبائي", "27", "المزامنة السحابية"],
            ["10", "التقويم الجبائي", "28", "الشركات التجريبية"],
            ["11", "التدقيق والمراجعة", "29", "اختبار المستخدمين"],
            ["12", "الأمان والمصادقة", "30", "ميزان المراجعة"],
            ["13", "مركز المساعدة", "31", "الشركاء والعملاء"],
            ["14", "التنبؤ المالي", "32", "الفواتير"],
            ["15", "المعايير المرجعية", "33", "المخزون"],
            ["16", "تحليل السيناريوهات", "34", "الرواتب والأجور"],
            ["17", "التدفقات النقدية", "35", "الميزانية"],
            ["18", "المقارنات المالية", "", ""],
        ],
    ...
    ar_para(doc, "شملت جلسات التطوير الأخيرة (من v3.1.3 إلى v3.1.8) إضافة مجموعة من الميزات المتقدمة التي تُعدّ جوهر الابتكار في هذا المشروع:")
    ...
    bullet(doc, "2020 اختباراً ناجحاً + تغطية وحدات 100%")

    heading(doc, "13.13 إصلاح مشاكل النسخة المثبّتة (v3.1.7)", 2)
    bullet(doc, "الشاشات الفارغة: الترقية القادمة من Setup-v3.1.6 وصلت ناقصة الملفات الحرجة (sqlite3.dll/_sqlite3.pyd + pandas/_libs + pytz/zoneinfo) — نُسخت الملفات المفقودة وتحقق آلي بتحميل كل الشاشات بلا استثناءات")
    ...
    heading(doc, "13.14 معالجة ملاحظات تحليل المشروع (2026-08-04)", 2)
    bullet(doc, "تنظيف git: إزالة ملفات الحالة والتقارير الملوّثة من التاريخ (-32,652 سطراً) + توسيع .gitignore (chat_history/login_session/db-sidecars/artifacts)")
    bullet(doc, "اختبارات أداء test_startup_perf.py (4): إقلاع 44ms + مشاهد 95-402ms + ذاكرة 45MB بحدود متسامحة")
    bullet(doc, "CI عبر GitHub Actions: ubuntu + Python 3.11 + xvfb-run pytest tests -q + تغطية معلوماتية فقط")
    bullet(doc, "تفكيك setup_ui الست العملاقة (350→27 / 261→15 / 224→28 / 217→31 / 212→24 / 192→9) إلى دوال _build_* — إعادة ترتيب صرفة بلا تغيير منطقي")
    bullet(doc, "طبقة تصدير موحدة ui/exporters.py (new_workbook/add_excel_sheet/ask_save_path/style_header_row/write_charts_pdf) + ترحيل 3 شاشات (لوحة التحكم المتقدمة/الرؤى الذكية/ربحية المراكز) −180 سطر boilerplate + 9 اختبارات")

    heading(doc, "13.15 تحسينات UI/UX", 2)
    ...
    heading(doc, "13.16 الشاشات المحاسبية الجديدة (شاشات 30–35)", 2)
    ar_para(doc, "أُضيفت 6 شاشات محاسبية في الإصدار v3.1.8 تُكمل الدورة المحاسبية الأساسية:")
    bullet(doc, "ميزان المراجعة (30): أرصدة الحسابات مدينة/دائنة مع تسوية وأستاذ عام")
    bullet(doc, "الشركاء والعملاء (31): تقادم ديون العملاء والموردين وتحليل الملاءة")
    bullet(doc, "الفواتير (32): إدارة الفواتير مع حساب TVA وتقارير دورية")
    bullet(doc, "المخزون (33): جرد وتقييم المخزون بمتوسط التكلفة المتحرك")
    bullet(doc, "الرواتب والأجور (34): كشف الرواتب مع خصم CNAS وIRG وكشف سنوي")
    bullet(doc, "الميزانية (35): مقارنة فعلية/مخططة وانحرافات وتوقعات")

    heading(doc, "13.17 نظام السنوات الجبائية (2025/2026)", 2)
    ar_para(doc, "أُضيف نظام إدارة السنوات الجبائية لتمكين المحاسب من العمل على الإعدادات الضريبية للسنة المختارة، استناداً إلى نشرة DGI الرسمية لسنة 2026:")
    bullet(doc, "ملفا tax_config_2025.json (مرجعي) وtax_config_2026.json (وفق النشرة الجديدة)")
    bullet(doc, "شرائح IRG 2026 محدّثة: 0% / 23% / 27% / 30% / 33% / 35%")
    bullet(doc, "إضافة IFU (0.5% / 5% / 12% بحد أدنى 30,000 دج)")
    bullet(doc, "رسم التكوين والتمهين (1%+1% خصم بالإنفاق)")
    bullet(doc, "اقتطاع الإيجارات (7%/15%/مؤقت 7% فوق 1.8M)")
    bullet(doc, "تبويب «سنوات النظام» في TaxView: مبدّل سنوات + محرر JSON + حاسبات")

    heading(doc, "13.18 الوحدات التجارية (Licensing + Encryption + Entitlement)", 2)
    ar_para(doc, "أُضيفت ثلاث وحدات تجارية في إطار حماية المنتج وتأمينه:")
    bullet(doc, "Module 1 – الترخيص (commercial/licensing/): مفاتيح RSA-2048 بتوقيع PKCS1v15 SHA-256 + تثبيت على عتاد المستخدم (MAC+CPU+Disk → SHA-256) + مهلة 14 يوماً ثم read-only + طبقات FREE/PRO/ENTERPRISE")
    bullet(doc, "Module 2 – التشفير (commercial/encryption/): Argon2id من cryptography المدمج + تنسيق SACF1 (AES-256-GCM: الـ tag يصادق الرأس كاملاً) + دعم توافق القديم PBKDF2")
    bullet(doc, "Module 3 – البوابات (commercial/entitlement.py): current_tier()/feature_allowed()/set_store() — بوابات مفعّلة: cloud_sync على PRO، ai_unlimited على ENTERPRISE")

    heading(doc, "13.19 حفظ الجلسة المشفّر (Remember Me)", 2)
    ar_para(doc, "ميزة «تذكرني» في شاشة الدخول تحفظ الجلسة بتشفير AES-256-GCM بمفتاح SHA-256 لبصمة العتاد (hardware_id.fingerprint):")
    bullet(doc, "كلمة السر لا تُكتب نصاً أبداً — تعمل على جهاز التخزين فقط")
    bullet(doc, "الجهاز الآخر: إكمال تلقائي للبريد وفشل آمن")
    bullet(doc, "حراسة تلقائية: لا تسجيل تلقائي عند must_change_password/2FA/كلمة خاطئة")
    bullet(doc, "تسجيل الخروج يمسح كلمة السر المخزّنة فقط (يبقي البريد)")

    heading(doc, "13.20 تنظيم وتوحيد الواجهة (v3.1.8)", 2)
    ar_para(doc, "شملت جلسات v3.1.8 تنظيم الواجهة بشكل شامل لمعالجة ملاحظات المستخدمين:")
    bullet(doc, "الشريط الجانبي المجمّع: 6 مجموعات (رئيسي/محاسبة/تحليل/جباية/أدوات/نظام) بعناوين غير قابلة للنقر")
    bullet(doc, "نافذة متكيفة: حجم النافذة يتكيف مع دقة الشاشة تلقائياً عبر availableGeometry()")
    bullet(doc, "شريط حالة 40px مع ملصق إصدار دائم v3.1.8")
    bullet(doc, "توحيد ارتفاعات العناصر: حقول وأزرار 40px، صفوف جداول 44px عبر كل الشاشات الـ35")
    bullet(doc, "توحيد الهوامش والتباعد: CARD_MARGINS (16,12,16,12) للبطاقات + (15,20,15,15) للمجموعات")
    bullet(doc, "إصلاح ترميز اللغة العربية في settings_view وإصلاح تداخل الحقول في tax_view")

    # ============================ 14. الاختبارات وضمان الجودة ============================
    heading(doc, "14. الاختبارات وضمان الجودة", 1)
    ar_para(doc, "تم تطوير 2020 اختباراً موزعة على 65 ملف اختبار، تغطي جميع مكونات النظام، عبر `python -m pytest tests -q`.")
    ...
    heading(doc, "14.2 ملفات الاختبار", 2)
    ar_para(doc, "يضم المشروع 65 ملف اختبار موزعة على فئات:")
    bullet(doc, "اختبارات الوحدات الأساسية: test_calculation.py (النسب المالية)، test_analysis.py (DuPont)، test_audit.py (التدقيق)، test_tax.py (الجباية)")
    bullet(doc, "اختبارات الميزات: test_cashflow.py، test_comparative.py، test_breakeven.py، test_budget.py، test_forecasting.py")
    bullet(doc, "اختبارات متقدمة: test_ai_insights.py، test_benchmarks.py، test_scenarios.py، test_cost_center.py، test_advanced_dashboard.py")
    bullet(doc, "اختبارات واجهات: test_ui.py، test_ui_views.py، test_ui_constants.py")
    bullet(doc, "اختبارات تكامل: test_integration_workflow.py، test_integration_database.py، test_integration_performance.py")
    bullet(doc, "اختبارات قبول: test_uat.py (رحلة كاملة بـ35 شاشة)")
    bullet(doc, "اختبارات تجارية: test_licensing.py، test_encryption.py، test_entitlement.py (تغطية 99%+)")
    bullet(doc, "اختبارات أداء: test_startup_perf.py")
    bullet(doc, "المجموع: 2020 اختباراً ناجحاً (python -m pytest tests -q)")
    ...
    bullet(doc, "i18n: 2018 مفتاحاً × 3 لغات (AR/EN/FR)")
    ...
    bullet(doc, "35 شاشة تفاعلية كاملة مع دعم RTL للعربية و3 لغات و3 ثيمات")
    ...
    bullet(doc, "2020 اختباراً ناجحاً مع تغطية 100% للوحدات")
    bullet(doc, "أداء محسّن: إقلاع 44ms وذاكرة 45MB")
    bullet(doc, "مثبت Inno Setup (66.9MB) ونسخة ZIP (109.4MB) وتحديث تلقائي مخفي")
    ...
    ar_para(doc, "تم تطوير المشروع وفق أفضل ممارسات هندسة البرمجيات، مع الالتزام بمبادئ الجودة والاختبار المستمر، مما نتج عنه 2020 اختبار وحدة ناجحاً وتغطية 100% للوحدات. كما تم توفير نظام تحديث تلقائي لتسهيل عملية التطوير والصيانة المستمرة.")
    ...
    heading(doc, "17.1 المدى القصير (قيد التطوير)", 2)
    bullet(doc, "1. وحدة المشتريات: إدارة طلبات الشراء وأوامر التوريد والموردين (شاشة 36)")
    bullet(doc, "2. دعم الفواتير الإلكترونية (E-Invoicing — شاشة 37)")
    bullet(doc, "3. المزامنة البنكية التلقائية: إطار API بنكي مع محاكي للتجربة دون اتصال")
    heading(doc, "17.2 المدى المتوسط", 2)
    bullet(doc, "1. توقيع رقمي للنسخ التنفيذية وتجاوز Smart App Control")
    bullet(doc, "2. تطوير نسخة سحابية (Cloud) مع قاعدة بيانات مركزية متعددة المستخدمين")
    bullet(doc, "3. إضافة تقارير مالية معيارية (IFRS، IAS)")
    bullet(doc, "4. تحليل متقدم باستخدام Machine Learning (scikit-learn) للتنبؤ بالأزمات المالية — ترقية من النماذج الإحصائية الحالية")
    heading(doc, "17.3 المدى البعيد", 2)
    bullet(doc, "1. تطوير تطبيق جوال (Mobile App) لمتابعة المؤشرات — معلق حالياً")
    bullet(doc, "2. الربط مع مصلحة الضرائب الجزائرية للتصريح الإلكتروني")
    bullet(doc, "3. دعم التوقيع الإلكتروني")
    bullet(doc, "4. منصة ذكاء اصطناعي متكاملة للتحليل المالي والتنبؤ — يتم تطويرها حالياً")
