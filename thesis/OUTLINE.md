...
### 5.5 Mobile Companion App (Flutter 3.44.7) — NEW
- **Motivation:** decision-makers need financial monitoring away from the office; the report's "future work" listed a mobile app — now implemented (Android, Flutter/Dart 3.12).
- **Architecture:** domain-driven `mobile/` — core (async ring-buffer logger / map-based i18n 280+ keys × AR/EN/FR RTL / Material 3 themes), data (snapshot loader, local cache, tax notifier), 5 features (Dashboard KPIs + trend chart + Z-Score + company card, 20 ratios, AI health score + risk radar + executive summary + recommendations, tax calendar, IAS summary) + CustomPainter widgets (radar/bar chart — no chart library).
