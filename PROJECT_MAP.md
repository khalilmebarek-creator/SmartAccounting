...
> **آخر حالة (2026-08-12):** v3.1.8+ — **2037 اختباراً** (سطح المكتب) + **43 اختباراً Flutter** (mobile/) + **تطبيق جوال Android** (arm64 18MB) + تغطية وحدات 100% + 2105 مفتاح i18n × 3 + **شريط Ribbon 7 تبويبات** + **إصلاح انهيار matplotlib** + **ML مفعّل كاملاً** + **تقارير IAS/IFRS** + **منصة ذكاء اصطناعي متكاملة**.
...
| 90 | 2026-08-12 | **تطبيق الجوال SmartAccounting Mobile (Goal: رفيق متابعة للمؤشرات على Android — Flutter 3.44.7)**: مجلد `mobile/` كامل — **M1** هيكل + 3 لغات (AR/EN/FR) RTL + ثيم M3 (11 widget test)؛ **M2** طبقة بيانات متوافقة مع سطح المكتب: تحليل wrapper snapshot (app/format/checksum sha256) + فك تشفير **SACF1** (Argon2id + AES-256-GCM عبر cryptography 2.9.0 — fixture حقيقي مولّد من سطح المكتب يُفك بنجاح، كلمة خاطئة/تلاعب/checksum مرفوضة = 12 اختباراً)؛ **M3** 5 شاشات (لوحة KPI + 20 نسبة + صحة AI برادار CustomPainter + تقويم جبائي + IAS) مع **اختبارات parity**: معادلات Health Score في Dart مطابقة حرفياً لـ modules/ai_platform.py (fixture health_expected.json = 6 اختبارات)؛ **M4** تنبيهات محلية قبل 3 أيام من الاستحقاقات (خطة pure + غلاف flutter_local_notifications 22.3 best-effort = 7 اختبارات)؛ **M5** APK: تثبيت JDK 17 + Android SDK 36 + إصلاح سلسلة بناء (AGP 9→8.9.1 بسبب تعارض file_picker/builtInKotlin مع flutter_local_notifications + Gradle 9.1→8.11.1 + Kotlin 2.3.20→2.1.20 + coreLibraryDesugaring) → **arm64 18.0MB** (<25MB) + v7a 15.6 + x86_64 19.4 — **43 اختباراً + analyze نظيف** + `mobile/PROJECT_MAP.md` (TECH_STACK/SYSTEM_FLOW/ARCHITECTURE/ORPHANS & PENDING) + مولد fixtures `tools/gen_mobile_fixtures.py` |
