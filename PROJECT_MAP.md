...
| 86 | 2026-08-07 | **تفعيل ML كاملاً دون بوابات (Goal: ذكاء اصطناعي متاح لكل المستخدمين)**: إزالة استدعاءات `feature_allowed("ai_unlimited")` من `ai_insights_view.py` (3 مواضع: `_enforce_gates` كان يقصر التنبؤ على 3 أشهر لـ FREE → أصبح 6 أشهر دائماً + `_export_pdf` + `_export_excel` كانا ممنوعين لغير ENTERPRISE → مفتوحان للجميع) — محرك `modules/ai_insights.py` لم يُمس + تعريف البوابة في tier.py باقٍ (لاختبارات البنية) + بوابة cloud_sync على PRO لم تُمس + تحديث 3 اختبارات توقع في test_entitlement.py → **2023 اختباراً أخضر** |
