...
| 93 | 2026-08-13 | **رفع الإصدار v3.2.0 (Goal: تحديث يصل لمستخدمي سطح المكتب + صفحة تحميل الجوال)**: رفع `config.py` APP_VERSION + i18n window_title ×3 + installer.iss + build_nuitka.py + pyproject.toml → **3.2.0** (درس متكرر: PowerShell replace أفسد العربية في i18n.py → استُعيد من git وأُعيد التعديل بـ Python io UTF-8 + BOM أُزيل من الملفات الخمسة) + docs/version.json (v3.2.0 + mobile_url جديد + 7 بنود changelog) + docs/index.html (softwareVersion + hero_badge + **بطاقة تحميل ثالثة لتطبيق الجوال** `dl_mob_*` + سجل v3.2.0 بـ upd_date10/upd_new36-38 + استعادة عنوان v3.1.8 للسجل الضريبي) + docs/script.js (hero_badge ×3 + upd_date10/upd_new36-38 ×3 لغات) + README + CHANGELOG (v3.2.0 أعلى السجل) + إعادة بناء Nuitka (exe) + Inno Setup (**Setup 63.2MB**) + ZIP محمول (**103.3MB**) + APK arm64 (**18.2MB**) + **GitHub release v3.2.0** بالأصول الثلاثة (gh CLI) → **2037 اختباراً أخضر** |
