...
> **آخر حالة (2026-08-07):** v3.1.8+ — **2029 اختباراً** + تغطية وحدات 100% + 2078 مفتاح i18n × 3 لغات + **شريط Ribbon أفقي** (6 تبويبات) + **إصلاح انهيار matplotlib بألوان rgba** + **إعادة تشكيل 3 شاشات محاسبية بعناوين فوق الحقول** + **ML مفعّل كاملاً** + **تقارير مالية IAS/IFRS** (4 تقارير: المركز المالي/الدخل/التدفقات النقدية/التغيرات في حقوق الملكية) + Module 1 ترخيص RSA-2048 + Module 2 تشفير AES-256-GCM/Argon2id + Module 3 بوابات طبقات + ميزة «تذكرني» المشفّرة.
...
| 87 | 2026-08-07 | **تقارير مالية معيارية IAS/IFRS (Goal: قوائم IAS 1 + IAS 7 من البيانات المالية)**: محرك `modules/ias_reports.py` جديد — يولّد 4 تقارير: **قائمة المركز المالي** (IAS 1: أصول متداولة/غير متداولة + حقوق ملكية + خصوم)، **قائمة الدخل الشامل** (IAS 1: إيرادات/تكلفة مبيعات/ربح تشغيلي/صافي الدخل)، **التدفقات النقدية** (IAS 7 الطريقة غير المباشرة: تشغيلي/استثماري/تمويلي)، **التغيرات في حقوق الملكية** (IAS 1) — يستخدم بيانات `state.financial_data` و `state.tax_summary` القائمة + تقدير بنود غير متاحة (اهتلاك 5%، تغير قروض 5%، نفقات رأسمالية 8% من الأصول غير المتداولة) + **شاشة جديدة 38** `ias_reports_view.py` (QTabWidget 4 تبويبات + جدولان بعمودي البيان/المبلغ + تصدير PDF بخط Amiri + تصدير Excel عبر openpyxl) + تسجيل في main_window (factory 38 + ribbon nav_group_analysis + اختصار Ctrl+Shift+G) + i18n **+64 ×3 = 2078 مفتاحاً** + **6 اختبارات** test_ias_reports.py → **2029 اختباراً أخضر** |
