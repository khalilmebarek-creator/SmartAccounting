...
| 92 | 2026-08-13 | **إصلاح تحميل snapshot من مزوّدي السحابة + جولدن حتمي (Goal: تجربة تركيب هاتف حقيقية سليمة)**: مستخدم ثبّت التطبيق على هاتفه وفشل تحميل snapshot من Gmail («فشل تحميل ملف») — الجذر: ملفات Gmail/Drive تُسلَّم كـ content-URI لا يقرؤها dart:io → **الإصلاح**: `FilePicker.pickFiles(withData: true)` يجلب البايتات مباشرة + `utf8.decode` لكل القراءات (كان `String.fromCharCodes` يشوّه العربية ويُفشل checksum) + **goldens حتمية**: نص «باقي X يوم» في التقويم الجبائي كان يعتمد على تاريخ اليوم → حقن ساعة ثابتة `TaxCalendarScreen(now:)` (840px اختلاف) + خادم تحميل محلي `PhoneShare` (http.server على 8080 + index.html عربي) لنقل APK/snapshot/صور للهاتف عبر WiFi — **53 اختباراً أخضر + APK arm64 18.2MB أعيد بناؤه وثُبّت فعلياً على هاتف المستخدم (تحقق ناجح)** |
