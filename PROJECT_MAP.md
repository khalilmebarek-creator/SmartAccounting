...
> **آخر حالة (2026-08-07):** v3.1.8+ — **2023 اختباراً** + تغطية وحدات 100% + 2014 مفتاح i18n × 3 لغات + معايير واجهات موحّدة + **شريط Ribbon علوي** (6 تبويبات، أزرار كل تبويب تحته، قابل للطي) + **إصلاح انهيار matplotlib بألوان rgba** (تحويل إلى hex في ThemeColors.get) + **إعادة تشكيل 3 شاشات محاسبية بعناوين فوق الحقول** (partners/invoicing/inventory عبر BaseView._labeled_field) + Module 1 ترخيص RSA-2048 offline + Module 2 تشفير AES-256-GCM + Argon2id + Module 3 بوابات طبقات مفعّلة + ميزة «تذكرني» المشفّرة. الجدول أدناه سجل تاريخي؛ أرقام الجلسات القديمة تعكس حالتها وقتها.
...
| 85 | 2026-08-07 | **شريط Ribbon + إصلاح انهيار matplotlib + إعادة تشكيل 3 شاشات محاسبية (Goal: واجهة واضحة بعناوين فوق الحقول)**: (1) **شريط ribbon** في main_window (QTabBar 6 تبويبات nav_group_* + QStackedWidget لوحات بأزرار الـ37 شاشة + زر طي ▲/▼) — بعد ملاحظتَي المستخدم أصبح التبويب في صف وأزراره في صف تحته (QVBoxLayout) مع أنماط `#ribbonTabs`/`#ribbonBtn` في الثيمات الثلاثة + مسح التبويبات/اللوحات عند إعادة البناء (تبديل اللغة) + حذف دالة ميتة `_on_ribbon_tab_changed`؛ (2) **إصلاح انهيار حقيقي 0xC0000409**: `ThemeColors.get()` كانت تُرجع `rgba(255,255,255,0.10)` لثيمي dark/modern ولا يقرؤها matplotlib → التطبيق يسقط عند رسم لوحة التحكم ببيانات (كشفه test_enter_data_calculate_propagates_state) — الحل تحويل rgba→#rrggbb داخل get() فيُصلح كل الرسوم دفعة واحدة (درس: ألوان الرسوم يجب أن تكون matplotlib-compatible لا CSS)؛ (3) **إعادة تشكيل 3 شاشات** حسب صور المستخدم («متداخلين وعنوان خانات مش فوقها»): مساعد مشترك `BaseView._labeled_field()` (QLabel صغير فوق كل حقل) + partners_view (إضافة شريك 5 حقول + معاملة جديدة 4 حقول) + invoicing_view (إنشاء فاتورة 4 + عناصر 3) + inventory_view (إضافة عنصر 8 حقول في صفّين + حركة مخزون 5 + إزالة addStretch من صف الإحصائيات فتمتد البطاقات بعرض كامل) + زر الإضافة في نفس الصف بمحاذاة `Qt.AlignBottom` + i18n **2014** (+inventory_price/inventory_min_qty ×3) + **3 اختبارات بنية جديدة** `test_form_fields_have_labels` في test_ui.py (TDD: فشلت 3 ثم نجحت) → **2023 اختباراً أخضر** |
