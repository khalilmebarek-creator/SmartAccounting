...
| 91 | 2026-08-12 | **تطوير تطبيق الجوال أكثر (Goal: رفيق عرض أقوى للدفاع)**: (1) **لوحة قيادة مطوّرة** — رسم أعمدة (إيرادات/مصاريف/أرباح) بـ CustomPainter + بطاقة Z-Score بحالة لونية (آمن/رمادي/خطر) + **أسهم اتجاه ▲▼%** مقارنة بالملف السابق (LocalStore يزيح last→prev عند الحفظ + AppState.prevSnapshot يُستعاد من القرص) + بطاقة معلومات الشركة القابلة للطي (NIF/RC/الشكل/العنوان/الهاتف/البريد/البنك من الـ payload)؛ (2) **ملخص تنفيذي + توصيات** في شاشة صحة AI — `ai_summary.dart` نسخة Dart بمطابقة حرفية مع `modules/ai_platform.py` (نفس العتبات ونفس الصياغة العربية) — **4 اختبارات parity**: السلاسل العربية مطابقة fixture سطح المكتب حرف-بحرف + توصيات بنفس الأولويات؛ (3) **صور شاشات للدفاع** — اختبار golden مع FontLoader بخط Amiri الحقيقي (360×780) → 10 PNG (5 شاشات × ar/en) في `mobile/screenshots/`؛ (4) **قسم الجوال في المذكرة** — thesis/OUTLINE.md: فقرة 5.5 (Mobile Companion: عمارة + توافق SACF1 + ضمان parity + تنبيهات + جودة) + تحديث المساهمات والأعمال المستقبلية (iOS/FCM)؛ إصلاح خطأ: `_setSnapshot` كانت تزيح prev المستعاد من القرص (الإزاحة فقط عند وجود حالي) + i18n mobile +57 ×3 = **280 مفتاحاً** → **53 اختباراً Flutter + analyze نظيف + APK arm64 18.2MB** |
