...
> **آخر حالة (2026-08-07):** v3.1.8+ — **2037 اختباراً** + تغطية وحدات 100% + 2105 مفتاح i18n × 3 لغات + **شريط Ribbon 7 تبويبات** (تحليل + تقارير منفصلان) + **إصلاح انهيار matplotlib** + **إعادة تشكيل 3 شاشات بعناوين** + **ML مفعّل كاملاً** + **تقارير IAS/IFRS** + **منصة ذكاء اصطناعي متكاملة** + **توحيد `_make_stat` في BaseView** (DRY عبر 9 شاشات).
...
| 89 | 2026-08-07 | **تحسينات هندسية 1-4 (ملاحظات المراجعة): (1) تقسيم تبويب الشريط**: "تحليل مالي" كان 13 زراً يفيض على 1280px → أُضيف تبويب جديد "تقارير" (breakeven/benchmarks/scenarios/IAS = 4 أزرار) وأصبح التحليل 9 أزرار (ratios…cost_profit) — المجموع 7 تبويبات؛ (2) **توحيد `_make_stat` في BaseView**: كانت مكررة تطابقاً في 9 شاشات (partners/invoicing/inventory/payroll/budgeting/einvoicing/ledger/cloud_sync/procurement) — نُقلت إلى `BaseView._make_stat()` (DRY) وأُزيلت النسخ المحلية؛ (3) **توصية git branches** في AGENTS.md: `feature/*` و `bugfix/*` لحماية main؛ (4) **Type hints** على دوال BaseView (إضافة `-> QLabel` لـ _make_header و `-> tuple` لـ _make_stat) + i18n 2105 (+nav_group_reports ×3) → **2037 اختباراً أخضر** |
