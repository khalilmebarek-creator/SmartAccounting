...
| 94 | 2026-08-13 | **تنبيه يوم التسديد + تدقيق أمني (Goal: البروتوكولات الأمنية الأربعة مطبّقة صراحة)**: (1) **جوال**: `buildNotificationPlan` يبعث الآن تنبيهين لكل استحقاق — تذكير قبل 3 أيام + **تنبيه يوم التسديد نفسه** (09:00، `tax_due_today` ×3 لغات، معرّفات فريدة +100000) — 8 اختبارات مُحدّثة (TDD أحمر ثم أخضر) → **56 اختباراً Flutter**؛ (2) **الدفاع الشامل عن المدخلات**: `print_manager.generate_report_html` كان يمرر اسم الشركة/العناوين/الخلايا/المحتوى إلى HTML **بلا escape** → ثغرة XSS حقيقية (4 اختبارات TDD حمراء ثم أُصلحت بـ `html.escape` لكل مدخل) + تحقق `settings_view` من مخطط URL (http/https فقط، رفض `file://` = SSRF — مفتاح `settings_api_url_invalid` ×3 + اختباران) + chat_view يستخدم setText (نص صريح) ✓ + الجوال: تحقق app-id + checksum ✓؛ (3) **سلامة قواعد البيانات**: تدقيق شامل — كل القيم عبر `?` معاملات؛ f-strings فقط بأسماء جداول داخلية من الـ schema (ليست مدخلات مستخدم) ✓؛ (4) **إدارة الأسرار**: صفر مفاتيح صلبة في المصدر (grep تدقيق) — SMTP/API مشفّران في الإعدادات ✓؛ (5) **مرونة API**: لا نقاط نهاية خادمية في المنتج (سطح مكتب offline + جوال محلي) — **N/A موثّق** + متطلب rate-limiting مسجّل للنسخة السحابية المستقبلية → **2043 اختباراً أخضر** |
